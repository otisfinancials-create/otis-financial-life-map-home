...
export const ListAccountsResponseItem = zod.object({
  ...
  "lastSyncAttemptedAt": zod.string().nullish().describe('When the connection\'s last sync ATTEMPT started (success or failure); null for manual accounts'),
  "lastSyncError": zod.string().nullish().describe('Sanitized message from the most recent failed sync attempt; null when the last attempt succeeded'),
  "lastSyncErrorCode": zod.string().nullish().describe('Plaid error code from the most recent failed sync attempt (e.g. ITEM_LOGIN_REQUIRED)'),
  "consecutiveFailures": zod.number().optional().describe('Number of sync attempts that have failed in a row for this connection (0 = healthy)'),
  "needsReauth": zod.boolean().optional().describe('The bank requires the user to re-authenticate this connection (Link update mode)'),
  "wasPlaidLinked": zod.boolean().optional().describe('True when the account is no longer linked to Plaid but once was (soft-unlinked by disconnect or item removal). Distinguishes \"manual because unlinked\" from \"manually created\".'),
