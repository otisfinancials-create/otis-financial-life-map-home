...
- **Bills** — page titled "Bills"; full CRUD for recurring bills with categories (incl. Pets, Cell Phone), frequency, due day, active/inactive. Bills have an `amountType` (`positive`/`negative`, default negative) — positive bills generate income forecast rows and display green +$ in the list. "Auto-pay" payment method displays as "Bank Draft" (stored value stays `auto-pay`). Editing is a split-panel: clicking a row (or Edit) opens an inline right panel (list 60% / form 40%, X to close) using the shared `BillForm` (extracted from `bill-dialog.tsx`; Add Bill remains a dialog). Pill view toggle "All Bills" / "Planned vs Actual" — the latter is a current-month snapshot table by category: Planned (monthly-equivalent of active bills), Actual (bill-linked forecast rows marked paid this month, non-missed), Remaining, with totals row. Loans auto-sync: creating/editing a loan auto-creates a "[Loan Name] Payment" bill (category Debt Payments) unless a matching bill exists (`loanMatchesBill` in `artifacts/api-server/src/lib/financial-dedup.ts`); loan POST/PATCH responses carry `billSync` for the toast.
- **Savings & Investments** — `/savings-investments` page (sidebar nav between Assets and Loans): summary card (total across savings/investment/brokerage accounts + MoM change vs prior month) and per-account cards with an editable `savingsGoal` (accounts column) + Carolina-blue progress bar. MoM comes from `GET /api/savings/summary`, which upserts the current month's total into `savings_snapshots` (one row per user per YYYY-MM) and compares against the prior month's snapshot (null until a prior month exists).
...
- **Budget** — `/budget` page: bills grouped by category in accordions with monthly-equivalent totals, inline amount editing (PATCH + forecast regenerate + toast).
