...
- Plaid (P1, Jul 2026): Link connection flow live (PLAID_ENV=production). `plaid_items` table stores access tokens server-side only (unique per user+item, upsert on reconnect); accounts get `plaidItemId`, `plaidAccountId`, `availableBalance`, `lastSyncedAt`. Link token requests only `transactions` as required product with liabilities/investments/identity as `optional_products` (Otis is read-only — never request `auth`, it targets money-movement accounts and causes "No eligible accounts" errors); `account_filters` limit to checking/savings, credit card, and all investment subtypes. Endpoints: POST `/plaid/create-link-token`, `/plaid/exchange-token` (imports/upserts accounts by plaidAccountId, fetches institution name/logo), `/plaid/disconnect` (keeps account as manual; orphaned plaid_items are itemRemove'd + deleted). Access tokens are never returned by any API or logged (`sanitizePlaidError`). Frontend: `components/accounts/plaid-connect-button.tsx` (react-plaid-link); Accounts page shows institution logo, "Connected via Plaid" badge, last synced, and a Disconnect from Plaid menu item.
- Plaid P2 sync engine (Jul 2026): `plaid_transactions` table (raw Plaid rows, unique `plaid_transaction_id`, Plaid sign convention positive = money out) + `transactions_cursor`/`last_synced_at` on `plaid_items`. `services/plaid-sync.ts`: transactionsSync cursor loop (count 100), upsert added+modified, delete removed, cursor persisted only after a full successful walk (safe replay). Endpoints: POST `/plaid/sync` (auth, syncs all user items), GET `/plaid/transactions` (auth, joined account name/type, `limit` ≤500), POST `/plaid/webhook` (public, mounted before `requireAuth`; verifies the `Plaid-Verification` ES256 JWT via `lib/plaid-webhook-verify.ts` — raw body captured in app.ts `express.json({verify})` — and has per-item in-flight lock + 30s debounce). `linkTokenCreate` passes the webhook URL from `REPLIT_DOMAINS`. Nightly cron (node-cron, 2:00 AM) syncs all users from `index.ts`. No UI for these transactions yet (P3).
